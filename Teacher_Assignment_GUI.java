...
public class Teacher_Assignment_GUI extends Teacher_Portal_Screen implements MouseListener {
	...
	private static boolean addBool = true, viewBool = false, seeBool = false, assigned;
	...
	public void mouseClicked(MouseEvent e) {
		if (e.getSource() == addAssignment) {
			...
			if (list.size() == 0) {
				assigned = false;
			} else if (Submissions.allSubmitted()) {
				assigned = false;
			...
			if (!assigned) {
				new AddAssignment();
				viewAssignment.setIcon(
						new ImageIcon(getClass().getResource("/CMS_Icons/Teacher Portal/Silver/WhiteEye.png")));
				seeSubmissions.setIcon(
						new ImageIcon(getClass().getResource("/CMS_Icons/Teacher Portal/Silver/WhiteEye.png")));
			} else {
